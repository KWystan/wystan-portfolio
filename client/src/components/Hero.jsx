  ...
  return (
    <section className="pt-20 pb-6">
      <div className="max-w-5xl mx-auto px-6 w-full border-l border-black/7 border-line-animate">
        ...
        >
          <div className="size-[96px] rounded-full border-2 border-black/12 overflow-hidden shrink-0 animate-float">
            <img src={profilePic} alt={hero.firstName} className="w-full h-full object-cover" />
          </div>

          <div className="flex flex-col mt-2 gap-0.5">
            <span className="font-serif text-lg text-black/50 leading-none">
              Hello, I'm
            </span>
        ...
        <div
          ...
          style={{ animation: `fade-up 0.25s var(--ease-out-expo) both`, animationDelay: `${DELAY_BASE * 1}ms` }}
        >
          {hero.socials.map((social) => (
            <a
              ...
              className="w-8 h-8 rounded-lg border border-black/12 flex items-center justify-center hover-gate:border-black/35 active:scale-[0.97] transition-all duration-150"
        ...
        {/* ── Contact info ────────────────────────────────────── */}
        <div
          className="flex flex-col gap-1.5 text-xs text-black/45"
          style={{ animation: `fade-up 0.25s var(--ease-out-expo) both`, animationDelay: `${DELAY_BASE * 2}ms` }}
        >
          <span className="flex items-center gap-1.5">
            <span className="material-symbols-outlined text-[13px] text-black/45">location_on</span>
            {hero.location}
          </span>
          <a href={`mailto:${hero.email}`} className="flex items-center gap-1.5 hover-gate:text-black/80 transition-colors duration-150 active:scale-[0.97] w-fit">
            <span className="material-symbols-outlined text-[13px] text-black/45">mail</span>
            {hero.email}
          </a>
          <span className="flex items-center gap-1.5">
            <span className="material-symbols-outlined text-[13px] text-black/45">language</span>
            {hero.website}
          </span>
        </div>
      </div>
